...
pub(super) fn new() -> NativeModule {
    NativeModule::new(NAME, |m| {
        ...
        m.func(
            "read",
            ArgSpec::builder().req("path").def("encoding", "utf8"),
            concat!("Read the entire contents of a file into a string\n",),
            |globals, args, _| {
                let mut args = args.into_iter();
                let pathval = args.next().unwrap();
                let path = Path::new(pathval.string()?);
                ...
                match encoding {
                    ...
                    Encoding::Raw => {
                        ...
                        Ok(globals.new_handle::<Vec<u8>>(data)?.into())
                    }
                }
            },
        )
        ...
        m.func("isfile", ["path"], "", |_globals, args, _| {
            let arg = args.into_iter().next().unwrap();
            let path = Path::new(arg.string()?);
                ...
                Ok(Value::from(NativeGenerator::new(
                    ...
                    move |_globals, _| {
                        if let Some(dirpath) = stack.pop() {
                            ...
                            if sort {
                                let mut entries = gentry!(gentry!(fs::read_dir(&dirpath))
                    ...
                    move |_globals, _| {
                        while files.is_empty() {
                            if let Some(dir) = dirs.pop() {
                                ...
                                if sort {
                                    let mut entries = gentry!(gentry!(fs::read_dir(&dir))
                                        .collect::<std::result::Result<Vec<_>, _>>(
                                    ));
                                    // sort in reverse order, so that when we pop from the stack
                                    // we get them in alphabetical order
